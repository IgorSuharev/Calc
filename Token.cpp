...
bool Token::hasCharacterValue(TokenType type) {
    auto searchAmongCharactersPredicate{
        [type](const pair<TokenType, char>& p) -> bool {
            return p.first == type;
        }
    };
    auto itChar{
        find_if(typeToCharacter_pairs.begin(), typeToCharacter_pairs.end(), searchAmongCharactersPredicate)
    };
...
string Token::typeToValue(TokenType type) {
    auto searchAmongCharactersPredicate{
        [type](const pair<TokenType, char>& pair) -> bool {
            return pair.first == type;
        }
    };
    auto itChar{
        find_if(Token::typeToCharacter_pairs.begin(), Token::typeToCharacter_pairs.end(), searchAmongCharactersPredicate)
    };
    if (itChar != Token::typeToCharacter_pairs.end()) {
        return string{} + itChar->second;
    }

    auto searchAmongStringsPredicate{
        [type](const pair<TokenType, string>& pair) -> bool {
            return pair.first == type;
        }
    };
    auto itString{
        find_if(Token::typeToString_pairs.begin(), Token::typeToString_pairs.end(), searchAmongStringsPredicate)
    };
...
TokenType Token::characterToType(char ch) {
    ...
    auto it{
        find_if(typeToCharacter_pairs.begin(), typeToCharacter_pairs.end(), searchPredicate)
    };
...
TokenType Token::stringToType(const string& str) {
    ...
    auto it{
        find_if(typeToString_pairs.begin(), typeToString_pairs.end(), searchPredicate)
    };
